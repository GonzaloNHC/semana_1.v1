...
public class Cajero {
    ...
    public static int depositar(int deposito){
        ...
        if(esNegativo(deposito)){
            throw new IngresoSaldoNegativoException(deposito);
        }
    ...
    public static int retirar(int retiro){
        ...
        if(esNegativo(retiro)){
            throw new RetiroNegativoException(retiro);
        }
        if(esNegativo(nuevoSaldo)){
            throw new IngresoSaldoNegativoException(nuevoSaldo);
        }
    ...
    public static void mostrarRegistro(){
        for (int indice = 0; indice < registroMovimientos.length; indice++){
            if (estaVacio(registroMovimientos[indice])){
                System.out.println("No se han realizado acciones de tipo: "+ MOVIMIENTOS[indice]);
                continue;
            }
